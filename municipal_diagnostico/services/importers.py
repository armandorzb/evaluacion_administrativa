...
def import_usuarios(rows: list[dict]) -> dict:
    ...
    for row in rows:
        ...
        try:
            ...
            requested_iso45001 = parse_optional_flag(row.get("acceso_iso45001"))
        ...
        if requested_iso45001 is True and role not in ISO45001_ALLOWED_ROLES:
            errors.append(f"Diagnóstico ISO 45001:2018 solo puede asignarse a administrador, revisor, evaluador o consulta: {email}")
            continue
        ...
        module_access = normalize_module_flags(
            role,
            acceso_diagnostico=requested_diagnostic,
            acceso_bienestar=requested_wellbeing,
            acceso_iso9001=requested_iso9001,
            acceso_live=requested_live,
            acceso_iso45001=requested_iso45001,
        )
        if (
        ...
        user.acceso_iso45001 = module_access["acceso_iso45001"]

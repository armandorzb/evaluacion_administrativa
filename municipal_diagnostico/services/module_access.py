...
MODULE_LABELS = {
    ...
    MODULE_ISO45001: "Diagnóstico ISO 45001:2018",
...
ISO45001_ALLOWED_ROLES = {"administrador", "revisor", "evaluador", "respondente", "consulta"}
...
def default_module_flags(role: str | None) -> dict[str, bool]:
    ...
    return {
        ...
        "acceso_iso45001": normalized_role == "administrador",
    }
...
def normalize_module_flags(
    ...
    acceso_iso45001: bool | None = None,
) -> dict[str, bool]:
    ...
    iso45001 = defaults["acceso_iso45001"] if acceso_iso45001 is None else bool(acceso_iso45001)

    if normalized_role not in WELLBEING_ALLOWED_ROLES:
        bienestar = False
    if normalized_role not in ISO9001_ALLOWED_ROLES:
        iso9001 = False
    if normalized_role not in ISO45001_ALLOWED_ROLES:
        iso45001 = False
...
def modules_for_user(user) -> list[str]:
    ...
    if getattr(user, "puede_acceder_iso45001", False):
        available.append(MODULE_ISO45001)
...
def landing_endpoint_for_user(user) -> str | None:
    ...
    if available[0] == MODULE_ISO45001:
        return "iso45001.dashboard"
...
def endpoint_for_module(user, module_slug: str) -> str | None:
    ...
    if module_slug == MODULE_ISO9001 and getattr(user, "puede_acceder_iso9001", False):
        return "iso9001.dashboard"
    if module_slug == MODULE_ISO45001 and getattr(user, "puede_acceder_iso45001", False):
        return "iso45001.dashboard"

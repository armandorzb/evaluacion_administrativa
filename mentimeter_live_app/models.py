...
class Session(TimestampMixin, db.Model):
    ...
    assets = db.relationship(
        ...
        back_populates="session",
        cascade="all, delete-orphan",
        order_by="PresentationAsset.created_at",
    )
...
class PresentationAsset(TimestampMixin, db.Model):
    """A presentation-scoped image stored outside the public static tree.

    ``storage_key`` is generated by the server and is deliberately opaque.  It
    is never derived from a supplied filename, which keeps the file-serving
    route independent from filesystem paths supplied by a client.
    """
    ...
    storage_key = db.Column(db.String(120), nullable=False, unique=True, index=True)
    ...
    session = db.relationship("Session", back_populates="assets")
